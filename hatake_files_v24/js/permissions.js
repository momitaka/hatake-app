...
document.getElementById('btn-admin-login').addEventListener('click',()=>{
  // _dataStrategyはjs/storage.js抽出まではwindow経由の一時ブリッジ
  if(window._dataStrategy==='session')return; // マーケットプレイス版はパスワード方式を使わない（Supabase Authのroleのみで判定）
  const pw=document.getElementById('s-admin-pw').value;
  if(pw===ADMIN_PASSWORD){
    permState.isAdmin=true;
    // セッション版はlocalStorageに管理者フラグを書かない（セッション限定）
    if(window._dataStrategy!=='session')localStorage.setItem('hatake_admin','1');
    permApply();
...
if(window._dataStrategy!=='session'){
...
// saveLS/updateFarmNameDisplayはjs/storage.js抽出まではwindow経由の一時ブリッジ
// closeSettingsは設定ダイアログセクション抽出まで、openMasterはjs/master-recipes.js抽出までの一時ブリッジ
document.getElementById('btn-save-farm-name').addEventListener('click',()=>{farmMeta.name=document.getElementById('s-farm-name').value.trim();window.saveLS();window.updateFarmNameDisplay();const btn=document.getElementById('btn-save-farm-name');btn.textContent='保存しました';setTimeout(()=>{btn.textContent='保存';},1200);});
document.querySelectorAll('input[name="farm-font"]').forEach(r=>{r.addEventListener('change',()=>{farmMeta.font=r.value;window.saveLS();window.updateFarmNameDisplay();});});
