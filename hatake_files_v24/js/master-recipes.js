...
export function updateMasterBackBtn(){
  ...
  if(navState.masterView==='detail'){
    ...
    btn.onclick=()=>{renderMasterList();showMasterView('list');};
  }else{
...
export function renderMasterDetail(){
  ...
  const CMP_GROUP_NEW='__new__';
  const cmpGroupSel=document.createElement('select');cmpGroupSel.style.cssText='font-size:var(--fs-sm);padding:5px 7px;border-radius:var(--border-radius-md);border:0.5px solid var(--color-border-secondary);background:var(--color-background-primary);color:var(--color-text-primary);width:100%;max-width:240px';
  const cmpGroupNone=document.createElement('option');cmpGroupNone.value='';cmpGroupNone.textContent='グループ化しない（このレシピ単独）';cmpGroupSel.appendChild(cmpGroupNone);
  [...new Set(Object.values(masterData.vegMaster).map(v=>v.compareGroup).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ja')).forEach(g=>{const o=document.createElement('option');o.value=g;o.textContent=g;cmpGroupSel.appendChild(o);});
  const cmpGroupAddNew=document.createElement('option');cmpGroupAddNew.value=CMP_GROUP_NEW;cmpGroupAddNew.textContent='＋ 新しいグループを作成';cmpGroupSel.appendChild(cmpGroupAddNew);
  cmpGroupSel.value=veg.compareGroup||'';
  if(!permCanEditFarm())cmpGroupSel.disabled=true;
  else cmpGroupSel.addEventListener('change',()=>{
    if(cmpGroupSel.value===CMP_GROUP_NEW){
      showTextPromptDialog('新しい比較グループ名','例: トマト',name=>{
        const trimmed=(name||'').trim();
        if(!trimmed){cmpGroupSel.value=veg.compareGroup||'';return;}
        masterData.vegMaster[veg.id].compareGroup=trimmed;saveLS();renderMasterDetail();
      },()=>{cmpGroupSel.value=veg.compareGroup||'';});
      return;
    }
    if(cmpGroupSel.value)masterData.vegMaster[veg.id].compareGroup=cmpGroupSel.value;
    else delete masterData.vegMaster[veg.id].compareGroup;
    saveLS();
  });
  cmpGroupBlock.append(cmpGroupLbl,cmpGroupDesc,cmpGroupSel);scroll.appendChild(cmpGroupBlock);
  const aiBanner=document.createElement('div');aiBanner.className='ai-banner';aiBanner.style.margin='0 0 12px';const hasRM=veg.phases&&veg.phases.length>0;
