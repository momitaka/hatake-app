      ...
      phase.tasks.forEach((task,ti)=>{
        ...
        if(!permRequireSupervisor()){nameIn.readOnly=true;dayIn.readOnly=true;memoIn.readOnly=true;urlIn.readOnly=true;upBtn.style.display='none';downBtn.style.display='none';delBtn.style.display='none';}else{
          ...
          delBtn.addEventListener('click',()=>{
            ...
            if(inUse){showConfirm(`このタスクを削除すると、この野菜を使用中の栽培エリアの記録が参照できなくなる場合があります。\n削除しますか？`,doDelete);return;}
            doDelete();
          });
        }
      });
...
export function renderMasterSaveBar(col,veg){
  ...
  const delBtn=document.createElement('button');delBtn.className='btn-delete-master';delBtn.innerHTML='<i class="ti ti-trash" style="font-size:var(--fs-sm)"></i>削除';delBtn.addEventListener('click',()=>{const usedCells=Object.values(gridState.cells).filter(c=>c&&c.crop===veg.id);if(usedCells.length>0){showAlert(`「${veg.name}」は現在${usedCells.length}つの栽培エリアで使用中のため削除できません。\n栽培エリアの管理画面から野菜を外すか、栽培エリアを削除してから再度お試しください。`);return;}showConfirm(`「${veg.name}」をレシピから削除しますか？`,()=>{delete masterData.vegMaster[veg.id];navState.masterVeg=null;saveLS();renderMasterList();renderMasterDetail();});});
