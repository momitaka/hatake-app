...
- **畑（トップ画面）** = 「作物区画マップ」と「管理中の作物」リストを表示するトップ画面（旧称：グリッド画面）
- **作物区画マップ** = トップ画面上部の、全マスを行列で一覧表示するエリア
- **マス** = 畑を構成する最小単位の升目。行（A,B,C…）×列（1,2,3…）の座標を持つ
- **作物区画** = 1つ以上の連続する「マス」に野菜を登録して栽培管理している単位（1登録操作＝1作物区画）。管理を完了すると作物区画は消え、マスは未登録状態に戻る。離れた作物区画同士は「連携」機能で工程表・収穫記録等のデータだけを共有できるが、作物区画自体は別々のまま
- **管理画面** = 作物区画の詳細管理画面（野菜管理画面）
