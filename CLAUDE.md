...
> **セッションタイトルは必ず日本語でつけること。ただしClaudeにはセッションタイトルをリネームするツールが無いため、Claudeが実施するのは「提案」まで。セッション開始時、チャット冒頭で `#061 ○○○` の形式で提案タイトルを1行提示し、ユーザーがそれをコピーして手動リネームする運用とする。**
> **提案タイトルの番号部分は、開発ログDB（データソースID: `724aa09b-932b-4384-85d2-d699dbce8230`）を「日付」降順で1件取得し、そこに入っている「セッション番号」（自動採番プロパティ）の値+1を、`#060` のようにゼロ埋め3桁で付与すること（引用・参照を楽にするための通し番号。Notionタスク管理DBの管理番号（TSK-1等）とは別物）。**
> **セッション内で本題と関係のない派生課題に気づいた場合、その場で着手せず、即座にNotionタスク管理DBに未着手タスクとして起票し、一言報告した上で本題の作業を継続すること。派生課題を別セッションとして都度切り出すと数が増えて追跡不能になるため、タスク管理DBを唯一の追跡先とする。**
...
- **畑（トップ画面）** = 「畑のマップ」と「管理中の作物」リストを表示するトップ画面（旧称：グリッド画面）
- **畑のマップ** = トップ画面上部の、全マスを行列で一覧表示するエリア
- **マス** = 畑を構成する最小単位の升目。行（A,B,C…）×列（1,2,3…）の座標を持つ
- **栽培区画** = 1つ以上の連続する「マス」に野菜を登録して栽培管理している単位（1登録操作＝1栽培区画）。管理を完了すると栽培区画は消え、マスは未登録状態に戻る。離れた栽培区画同士は「連携」機能で工程表・収穫記録等のデータだけを共有できるが、栽培区画自体は別々のまま
- **管理画面** = 栽培区画の詳細管理画面（野菜管理画面）
- **栽培記録タブ** = 管理画面の4番目のタブ（旧称：作業記録タブ）。作業期間・合計収穫量などの統計、週別収穫グラフ、全体メモ、作業記録（工程実施の記録）・収穫記録を時系列でまとめた履歴を表示する。「作業記録」という語自体は工程実施の記録データを指す語として従来通り使用する（タブ名としては使わない）
